...
if (! function_exists('et_date')) {
     ...
     */
    function et_date(Carbon|string|null $date, ?string $gcFormat = null): string
    {
        if ($date === null || $date === '') {
            return '—';
        }
        ...
        $format = $gcFormat ?? (string) Setting::get('app.date_format', 'd M Y');

        if (app()->getLocale() === 'am') {
            return EthiopianCalendar::formatGc($carbon, $format);
        }
